...
public class StopPlaceVersionedSaverService {
    ...
    public StopPlace saveNewVersion(StopPlace existingVersion, StopPlace newVersion, Instant defaultValidFrom, Set<String> childStopsUpdated) {
        ...
        logger.info("Stop place {} change detection: stopPlaceChanged={}, hasExistingVersion={}", 
            newVersion.getNetexId(), stopPlaceChanged, existingVersion != null);
     ...
     * Excludes quays and children from comparison - only checks stop place fields.
     * 
    ...
    private boolean hasStopPlaceChanged(StopPlace existingStopPlace, StopPlace newStopPlace) {
        ...
        try {
            var differences = genericObjectDiffer.compareObjects(existingStopPlace, newStopPlace, stopPlaceDiffConfig);
            logger.info("Stop place {} comparison found {} differences: {}", 
                newStopPlace.getNetexId(), differences.size(), differences);
