...
public class InfoSpotsUpdater implements DataFetcher {
    ...
    private void incrementVersionForLinkedEntities(InfoSpot savedInfoSpot) {
        ...
        try {
            InfoSpotLocationRef locationRef = savedInfoSpot.getLocationRefs().iterator().next();
     ...
     * @throws IllegalStateException if parent reference is invalid
     */
    private StopPlace getTopLevelStopPlace(StopPlace stopPlace) {
        ...
        if (parentVersion == null) {
            logger.warn("StopPlace {} has parent reference with null version, using child", stopPlace.getNetexId());
            return stopPlace;
        }
        ...
        logger.warn("StopPlace {} has parent reference but parent not found: {}, using child", 
            stopPlace.getNetexId(), parentRef);
        return stopPlace;
    }
}
