...
public class QuayViewTests extends TiamatIntegrationTest {
    ...
    public void viewShowsData_withAlternativeName() throws Exception {
        ...
        List<Map<String, Object>> quayNewestVersions = queryAll();
        @NotNull List<String> alternativeName = Arrays.stream(((Object[]) quayNewestVersions.getFirst().get("alternative_name")))
                .map(an -> (String) an)
                .toList();

        assertThat(alternativeName.contains("quay-test-alternative-name"), is(notNullValue()));
    }
