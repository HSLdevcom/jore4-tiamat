...
public class QuayViewTests extends TiamatIntegrationTest {
    ...
    public void viewShowsData_withInstalledEquipment() throws Exception {
        DataBuilder.TestData testData = this.dataBuilder
                ...
                .asPersisted();
        verifyTestData(testData);
        assertThat(countQuayInstalledEquipment(), Matchers.is(2L));

        List<Map<String, Object>> quayNewestVersions = queryAll();
        String equipments = quayNewestVersions.getFirst().get("place_equipments").toString();

        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        objectMapper.setPropertyNamingStrategy(PropertyNamingStrategy.SNAKE_CASE);

        List<InstalledEquipment_VersionStructure> installedEquipments = objectMapper.readValue(equipments, new TypeReference<List<TicketingEquipment>>() {
        });
        assertThat(installedEquipments.getFirst().getNetexId(), CoreMatchers.equalTo(testData.installedEquipments.getFirst().getNetexId()));
    }
    ...
    private Long countQuayInstalledEquipment() {
        return (Long) entityManager.createNativeQuery("SELECT count(*) FROM quay_installed_equipment")
                .getResultList().getFirst();
    }
