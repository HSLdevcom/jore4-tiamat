...
The parameter `topographicPlaceExportMode` can be set to _NONE_, _RELEVANT_ or _ALL_
Relevant topographic places will be found from the exported list of stop places.

### Tariff Zone export mode

The parameter `tariffZoneExportMode` can be set to _NONE_, _RELEVANT_ or _ALL_
...
The parameter `groupOfStopPlacesExportMode` can be set to _NONE_, _RELEVANT_ or _ALL_
...
_Most of the parameters from synchronous export works with asynchronous export as well!_
...
### Import NeTEx file without _NSR_ IDs
...
By default, SecurityContextHolder use DEFAULT_LOCAL_STRATEGY. When using INITIAL importType, you should tell Spring to use MODE_INHERITABLETHREADLOCAL for SecurityContextHolder, allowing Spring to duplicate Security Context in spawned threads.
...
Tip: GraphiQL UI available on https://api.dev.entur.io/graphql-explorer/stop-places using _GraphiQL_:
